...
class DiscordStub(object):
  ...
  def __init__(self, channel):
    ...
    Args:
      channel: A grpc.Channel.
    """
    self.LinkDiscordAccount = channel.unary_unary(
        '/grpc_testauth.Discord/LinkDiscordAccount',
        request_serializer=grpc__testauth_dot_discord__pb2.LinkDiscordUserRequest.SerializeToString,
        response_deserializer=grpc__testauth_dot_discord__pb2.LinkDiscordUserResponse.FromString,
        )
...
class DiscordServicer(object):
  ...
  def LinkDiscordAccount(self, request, context):
    # missing associated documentation comment in .proto file
    pass
    context.set_code(grpc.StatusCode.UNIMPLEMENTED)
    context.set_details('Method not implemented!')
...
def add_DiscordServicer_to_server(servicer, server):
  rpc_method_handlers = {
      'LinkDiscordAccount': grpc.unary_unary_rpc_method_handler(
          servicer.LinkDiscordAccount,
          request_deserializer=grpc__testauth_dot_discord__pb2.LinkDiscordUserRequest.FromString,
          response_serializer=grpc__testauth_dot_discord__pb2.LinkDiscordUserResponse.SerializeToString,
      ),
  }
  generic_handler = grpc.method_handlers_generic_handler(
      'grpc_testauth.Discord', rpc_method_handlers)
  server.add_generic_rpc_handlers((generic_handler,))
